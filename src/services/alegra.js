...
import AlegraServiceHelper from '../helpers/SiigoServiceHelper.js'
import { alegraMapper } from '../mappers/alegraProduct.js'
import CSV from '../utils/csv.js'

class AlegraProduct {
  static async getProductsAlegra (body) {
    const { username, accessKey } = body
    const token = { username, accessKey }
    const productsData = []

    const { data } = await AlegraServiceHelper.getProducts({
      token,
      filters: { order_direction: 'DESC', order_field: 'id' }
    })
  ...
  static async fetchProducts ({ link, token }) {
    ...
    while (url) {
      const { status: statusCode, data } = await AlegraServiceHelper.getProductsNext({
        url,
        token
      })
